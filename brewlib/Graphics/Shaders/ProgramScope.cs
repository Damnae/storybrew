...
namespace BrewLib.Graphics.Shaders
{
    ...
    {
        private readonly List<ShaderType> types = new List<ShaderType>();
        private readonly List<ShaderVariable> varyings = new List<ShaderVariable>();
        private readonly List<ShaderVariable> uniforms = new List<ShaderVariable>();

        private int lastId;
        private string nextGenericTypeName => $"t_{lastId++:000}";
        private string nextGenericVaryingName => $"v_{lastId++:000}";
        ...
        public void DeclareTypes(StringBuilder code)
        {
            foreach (var type in types)
            {
                ...
                foreach (var field in type.Fields)
                    code.AppendLine($"    {field.ShaderTypeName} {field.Name};");
                code.AppendLine("};");
            }
        }
