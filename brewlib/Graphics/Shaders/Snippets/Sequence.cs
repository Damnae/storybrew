...
namespace BrewLib.Graphics.Shaders.Snippets
{
    ...
    {
        private ShaderSnippet[] snippets;
        ...
        public override IEnumerable<string> RequiredExtensions
        {
            get
            {
                foreach (var snippet in snippets)
                    foreach (var requiredExtension in snippet.RequiredExtensions)
        ...
        public override int MinVersion
        {
            get
            {
                ...
                foreach (var snippet in snippets)
                    if (snippet.MinVersion > minVersion)
                        minVersion = snippet.MinVersion;
                return minVersion;
            }
        }

        public Sequence(params ShaderSnippet[] snippets)
        {
        ...
        {
            foreach (var snippet in snippets)
                snippet.GenerateFunctions(code);
        }
